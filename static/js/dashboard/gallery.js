...
document.getElementById('searchGalleryInput').addEventListener('input', filterGalleryTable);

document.getElementById('clearGalleryInput').addEventListener('click', function () {
    document.getElementById('searchGalleryInput').value = '';
    filterGalleryTable();
});
...
function confirmDeleteGallery(galleryId, galleryName) {
    Swal.fire({
        title: 'Are you sure?',
        text: `Do you want to delete the gallery '${galleryName}'?`,
        icon: 'warning',
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function validateForm() {
        var nameInput = document.getElementById('galleryName').value.trim();
        if (nameInput === '') {
            Swal.fire({
                title: 'Error!',
                text: 'Please enter the gallery name.',
                icon: 'error',
        ...
        if (validateForm()) {
            ...
            .then(response => {
                if (response.ok) {
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: 'Failed to add gallery. Please try again later.',
                        icon: 'error',
                        confirmButtonText: 'OK'
                    });
                }
    ...
    document.getElementById('saveGalleryBtn').addEventListener('click', confirmFormSubmission);

    // Attach the form submission function to the Enter key press event
    document.addEventListener('keydown', function (event) {
        if (event.key === 'Enter' && document.getElementById('galleryPopup').classList.contains('show')) {
